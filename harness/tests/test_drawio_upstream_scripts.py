"""Ported upstream test suite from drawio-skill@765a95b (Agents365-ai, MIT).

Covers invariant correctness: shapesearch soundex/title-ranking, aiicons
variant-resolution, encode_drawio_url CJK+% round-trip, autolayout palette
+ group-tint + escape, validate CLI exit-codes + geometry checks, and all
5 importers (py/js/go/rust + pyclasses).

Paths adapted for harness skill layout: REPO_ROOT/harness/plugins/hs/skills/drawio/{scripts,data}.
No dependency on Graphviz/draw.io — autolayout exercised via to_drawio()
...
DRAWIO = os.path.join(REPO_ROOT, "harness", "plugins", "hs", "skills", "drawio")
SCRIPTS = os.path.join(DRAWIO, "scripts")
DATA = os.path.join(DRAWIO, "data")
...
class TestShapeSearch(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        # Strip source tag added by P3 catalog merge (not in upstream index)
        for s in cls.shapes:
            s.pop("source", None)
