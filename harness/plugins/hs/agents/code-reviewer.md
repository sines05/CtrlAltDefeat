...
skills: [code-review, scout, repomix]
memory: project
...
## Complexity lens (report separately from defects)

Alongside defect hunting, apply a **complexity-only** lens to new code: ask whether a lower
rung of the minimal implementation ladder does the same job — **Delete** (remove the need),
**Standard library** (stdlib-native), **Existing** dependency or utility (reuse), **Tiny**
change (a one-line edit), **Shrink** (the minimal version). This lens does **not** override a
correctness, security, a11y, scope, or error-handling finding, and its notes report
**separately from defects** — never block a merge on a complexity note alone.
...
**IMPORTANT**: Load and apply checklists from `hs:code-review`'s `references/checklists/` per `references/checklist-workflow.md` when: pre-landing review (`/hs:ship` or explicit request), OR significant changes (10+ files or security-sensitive). Two-pass model: critical (blocking) + informational (non-blocking).
...
- For full codebase: use `hs:repomix` to compact, then analyze
...
| Complexity | Lower-rung alternatives (Delete/Standard library/Existing/Tiny/Shrink); never overrides correctness/security |
...
- Respect `harness/rules/harness-contract.md` and `docs/code-standards.md`
...
Use the naming pattern from the `## Naming` section injected by hooks. If plan file given, extract plan folder first.
