...
# Reuse the centralized .env parser (same resolution resolve_env uses) rather than
# rolling a private one; fall back to the local parser if that module is absent.
sys.path.insert(0, str(Path(__file__).resolve().parent.parent.parent.parent / "scripts"))
try:
    from resolve_env import dotenv_values as _shared_parse_env
except ImportError:
    _shared_parse_env = None
...
class EnvLoader:
    ...
    def _find_repo_root(start: Path) -> Optional[Path]:
        ...
        for _ in range(20):  # guard against infinite loop on weird mounts
    ...
    def load_env_files() -> Dict[str, str]:
        ...
        Order (most-specific wins): process env > skill/.env > skills/.env > repo-root .env.
        Files load lowest-priority first and dict.update() lets the closer .env override.
        ...
        # Lowest: repo-root .env (the harness anchor; no install-output dir dependency).
        repo_root = EnvLoader._find_repo_root(script_dir)
        search_paths = []
        if repo_root is not None:
            search_paths.append(repo_root / ".env")          # repo-root .env
        search_paths.append(script_dir.parent.parent / ".env")  # skills/.env
        search_paths.append(script_dir.parent / ".env")         # skill/.env (repomix/.env)
    ...
    def _parse_env_file(path: Path) -> Dict[str, str]:
        ...
        Returns:
            Dictionary of environment variables
        """
        if _shared_parse_env is not None:
            return {k: v for k, v in _shared_parse_env(path).items() if v is not None}
        env_vars = {}
        try:
