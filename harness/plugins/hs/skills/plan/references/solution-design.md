...
- Every finalized decision still needs a `file:line` anchor per `harness/rules/verification-mechanism.md`; an item on this checklist with no evidence is `[ASSUMED]` (or `[PRIOR]` if it rests on prior/training knowledge), not a settled design choice.
