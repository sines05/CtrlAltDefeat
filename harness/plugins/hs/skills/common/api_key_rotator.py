...
class KeyRotator:
    ...
    def __init__(
        ...
        if verbose:
            print(f"[ok] KeyRotator initialized with {len(keys)} key(s)", file=sys.stderr)
    ...
    def get_key(self) -> Optional[str]:
        ...
        if available is None:
            if self._verbose:
                print("[warn] All API keys are on cooldown", file=sys.stderr)
            return None
    ...
    def mark_rate_limited(self, error_msg: str = "") -> bool:
        ...
        if self._verbose:
            print(
                f"[warn] Key #{self._current_index + 1} rate-limited, "
    ...
    def reset_cooldowns(self) -> None:
        ...
        if self._verbose:
            print("[ok] All cooldowns cleared", file=sys.stderr)
