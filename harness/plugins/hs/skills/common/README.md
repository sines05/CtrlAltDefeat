...
4. **Shared skills `.env` file** (shared across skills in the plugin)
   ```bash
   echo 'GEMINI_API_KEY=your-api-key' > <plugin>/skills/.env
   ```

5. **Skill directory `.env` file**
   ```bash
   echo 'GEMINI_API_KEY=your-api-key' > <plugin>/skills/your-skill/.env
   ```
