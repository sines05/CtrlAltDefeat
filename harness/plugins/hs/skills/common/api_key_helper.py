...
def find_api_key(skill_dir: Optional[Path] = None) -> Optional[str]:
    ...
    if api_key:
        print("[ok] Using API key from environment variable", file=sys.stderr)
        return api_key
    ...
    if project_env.exists():
        ...
        if api_key:
            print(f"[ok] Using API key from {project_env}", file=sys.stderr)
            return api_key
    ...
    if claude_env.exists():
        ...
        if api_key:
            print(f"[ok] Using API key from {claude_env}", file=sys.stderr)
            return api_key
    ...
    if shared_skills_env.exists():
        ...
        if api_key:
            print(f"[ok] Using API key from {shared_skills_env}", file=sys.stderr)
            return api_key
    ...
    if skill_env.exists():
        ...
        if api_key:
            print(f"[ok] Using API key from {skill_env}", file=sys.stderr)
            return api_key
...
def load_env_file(env_path: Path) -> Optional[str]:
    ...
    Returns:
        API key or None
    """
    return load_env_var(env_path, 'GEMINI_API_KEY')
...
def get_api_key_or_exit(skill_dir: Optional[Path] = None) -> str:
    ...
    if not api_key:
        ...
        print("\n[info] Please set your API key using one of these methods (in priority order):", file=sys.stderr)

        if skill_dir is None:
            skill_dir = Path(__file__).parent.parent
        project_dir = skill_dir.parent.parent.parent

        print("\n1. Environment variable (recommended for development):", file=sys.stderr)
        ...
        print("\n4. Shared skills .env file (shared across skills):", file=sys.stderr)
...
def get_client(skill_dir: Optional[Path] = None):
    ...
    if vertex_config['use_vertex']:
        ...
        if not vertex_config['project_id']:
            ...
            print("\n[info] Set your GCP project ID:", file=sys.stderr)
        ...
        print(f"[ok] Using Vertex AI endpoint", file=sys.stderr)
...
if __name__ == '__main__':
    ...
    print(f"[ok] Found API key (length {len(api_key)})")
