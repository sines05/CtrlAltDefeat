...
"""
    ...
    def test_load_env_files(self, auth_init, mock_project_root):
        """Test loading environment variables: project .claude/.env tier + process override.

        The skill-local and plugin-shared `.env` tiers are resolved relative to the
        script's own location (module-relative), so only the project `.claude/.env`
        tier and the process-env override are asserted here.
        """
        # Create the project-level .claude/.env (mockable via project_root)
