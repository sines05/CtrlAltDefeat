...
class BetterAuthInit:
    ...
    autoSignIn: true
      ...
      sendMagicLink: async ({ email, url }) => {
        ...
        throw new Error('magicLink sendMagicLink not implemented — replace this stub before deploying');
      }
    })""")
...
"""
    ...
    def run(self) -> None:
        ...
        print("\n--- .env ---")
        print("(Secrets generated — values omitted from display. Save to write them to disk.)")
    ...
    def _save_files(self, auth_config: str, env_content: str) -> None:
        ...
        if env_path.exists():
            # Timestamped backup so a second init never silently overwrites an
            # earlier backup (and won't collide with an existing file on Windows).
            from datetime import datetime
            ts = datetime.now().strftime("%Y%m%d_%H%M%S")
            backup = self.project_root / f".env.backup.{ts}"
            env_path.rename(backup)
            print(f"Backed up existing .env to {backup}")

        env_path.write_text(env_content, encoding="utf-8")
        os.chmod(env_path, 0o600)  # secrets file: owner-only
