...
from encoding_utils import (configure_utf8_console, emit_json, dumps_json,
                            replace_lone_surrogates)
from frontmatter_parser import parse_file
# ID_PATTERN_BY_TYPE is a DELIBERATE re-export: check_consistency.py imports
# it from this module (not from id_grammar directly), so this is a real
# cross-module consumer, not a dead import. COMP_ID_PATTERN has no such
# consumer here (check_consistency_competition.py imports it straight from
# id_grammar) and stays dropped.
from id_grammar import ID_PATTERN_BY_TYPE  # noqa: F401 — re-exported
    ...
    for line in body.splitlines():
        ...
        if line.startswith("# "):
            ...
            if node_id and "—" in h:
                ...
                if node_id in tail.split():  # whole-token, not substring (PRD-AUTH-E1 ⊄ PRD-AUTH-E10)
                    h = head.strip() or h
            return h
    return ""
...
def build_nodes(artifacts: List[Dict[str, Any]], product_dir: Path) -> List[Dict[str, Any]]:
    ...
    # `product_dir` is invariant across the loop — resolve() it once (each call is a
    # syscall chain of path normalisation/readlink) instead of per artifact.
    resolved_product_dir = product_dir.resolve()
    for art in artifacts:
        if not art["ok"]:
            continue
        fm = art["frontmatter"]
        try:
            rel = Path(art["file"]).resolve().relative_to(resolved_product_dir).as_posix()
        except ValueError:
            # A committed symlink whose target escapes docs/product/ (mode 120000
            # pointing outside the tree). Do NOT follow the escape into the graph
            # (foreign file content) and do NOT crash build_graph with a raw
            # traceback — every analytical spec CLI funnels through here and owes an
            # always-exit-0 contract. Skip the escaping artifact.
            continue
        node_type = _node_type(art)
        if node_type == "brd":
            # A hand-edited scalar `goals: 5` would make `for g in goals` raise
            # TypeError and crash build_graph (which every spec:* script funnels
            # through) — guard to a list so a malformed field yields no goals,
            # not a raw traceback.
            goals = fm.get("goals")
...
# Hard cap on the number of container nodes any single fingerprint/dedup walk
# will visit. A real artifact's `parts` (body + acceptance_criteria, or one
# risk/competitor entry) is at most a few hundred nodes, so no genuine content
# ever approaches this — but a YAML *alias* fanned out across siblings
# (`a: &a […]`, `b: [*a, *a, …]`, chained) is a DAG whose EXPANSION is
# O(fan**depth): a <1KB frontmatter file yaml-loads to a small object graph that
# a naive walk (and the json.dumps that follows) blows up to 10^8+ nodes, hanging
# build_graph and thus strict_gate — a real DoS on the gate this file feeds. The
# budget truncates that expansion to a stable marker, matching the work-budget
# precedent in check_traceability.py (_MAX_SCC_STEPS / _MAX_CYCLES_PER_SCC).
_FINGERPRINT_NODE_BUDGET = 100_000
_TRUNCATED_MARKER = "[content truncated: fingerprint node budget exceeded]"


def _stringify_keys(obj: Any, _seen: frozenset = frozenset(),
                    _budget: Optional[List[int]] = None) -> Any:
    """Recursively coerce every dict key to str, and every set/frozenset to a
    SORTED list of stringified elements. A hand-edit can leave a mixed
    int/str-keyed mapping nested under `acceptance_criteria` or a BRD goal's
    `metrics` (e.g. `- 1: a\\n  b: c`); json.dumps(sort_keys=True) raises
    TypeError comparing str<int on such a dict, which would crash build_graph
    for every spec:* script that funnels through it. The fingerprint only needs
    determinism, not key-type fidelity, so this loses nothing that matters:
    same input structure always coerces to the same str keys.

    The set/frozenset branch closes a companion gap: a YAML `!!set` value
    (a BRD goal's `metrics: !!set {...}`) yaml-loads to a Python `set`, which
    left unhandled falls through to `json.dumps(default=str)` -> `str(set)`,
    whose iteration order is PYTHONHASHSEED-randomized per process — the same
    content would fingerprint differently run to run. Sort AFTER stringifying
    (not before) so a mixed-type set never raises a str<int TypeError.

    A YAML anchor cycle (`acceptance_criteria: &a [x, *a]`) yaml-loads to a
    self-referential list/dict; without a guard the recursion never bottoms out
    and a RecursionError crashes build_graph (and thus strict_gate). `_seen`
    tracks the ancestor path by `id()` (a true back-edge → marker) — mirrors
    `_safe_jsonify`. Distinct from a cycle, a shared alias fanned out across
    siblings (`b: [*a, *a, …]`) is a DAG whose expansion is exponential; `_budget`
    (a shared mutable counter, decremented per container) caps the total node
    visits so that expansion — and the json.dumps that serializes it — cannot hang
    the gate, emitting a stable truncation marker once the cap is crossed. Real
    content stays far under the cap, so its fingerprint is byte-for-byte unchanged;
    only a pathological alias bomb is ever truncated (deterministically — walk
    order is fixed, so the same input always truncates to the same marker).

    Not injective on a pathological same-string mixed-type key/element
    (`1` and `"1"` collapse to one `"1"`) — out of scope: the fingerprint's
    job is deterministic change-detection of real content, not shape fidelity,
    and real content is never actually mixed-type like that."""
    if _budget is None:
        _budget = [_FINGERPRINT_NODE_BUDGET]
    ...
    if isinstance(obj, dict):
        ...
        _seen = _seen | {id(obj)}
        return {str(k): _stringify_keys(v, _seen, _budget) for k, v in obj.items()}
    if isinstance(obj, (set, frozenset)):
        return sorted(str(_stringify_keys(v, _seen, _budget)) for v in obj)
    if isinstance(obj, list):
        if id(obj) in _seen:
            return "[circular reference]"
        _seen = _seen | {id(obj)}
        return [_stringify_keys(v, _seen, _budget) for v in obj]
    return obj
...
def _content_fingerprint(parts: List[Any]) -> str:
    ...
    sorted for stability — _stringify_keys makes that sort possible even on a
    hand-edited mixed-key dict (see its docstring)."""
    canon = json.dumps(_stringify_keys(parts), ensure_ascii=False, sort_keys=True, default=str)
...
def _node_from_artifact(fm: Dict[str, Any], file_rel: str, node_type: Optional[str],
    ...
    raw_depends_on = fm.get("depends_on")
    node = {
        "id": nid,
        "type": node_type,
        # Content fingerprint of the artifact body (sha256, first 8 hex). It is
        # the cache key the memory layer keys off AND the signal that would let
        # an (unshipped) impact-pass detect a body-only edit (frontmatter
        # unchanged) — which the frontmatter-only fields below would otherwise
        # miss. Deterministic: same body bytes → same hash. See CHANGED_FIELDS /
        # changed_nodes.
        ...
        "size": fm.get("size"),
        # Raw pass-through (no `or []`), matching `risks` below: coercing a falsy
        # scalar (`personas: 0`) to [] here would hide it from check_consistency's
        # LIST_FIELDS invalid_type check, which needs the real (non-list) value to
        # flag it. LIST_FIELDS's `if v is None: continue` guard still treats an
        # absent field as no-personas, not a finding.
        ...
        "file": file_rel,
        # Raw pass-through (no `or []`) — see the `personas`/`metrics` comment above;
        # same LIST_FIELDS invalid_type rationale applies here.
        "brd_goals": fm.get("brd_goals"),
        ...
        # A bare scalar `depends_on: PRD-X` is coerced to [] by _as_id_list so the
        # adjacency/render consumers (which do `sorted(n.get("depends_on") or [])`)
        # never char-split a bare string. The malformed raw is preserved below under
        # `depends_on_invalid` so check_depends_on_type can still flag invalid_type —
        # otherwise the edge vanishes with NO finding and passes --strict clean,
        # unlike the sibling id-lists brd_goals/serves.
        "depends_on": _as_id_list(raw_depends_on),
    ...
    # Marker present ONLY on a malformed (non-list, non-None) depends_on — the
    # coerced `depends_on` above hid the shape from the LIST_FIELDS check, so this
    # carries the real value forward for check_depends_on_type to flag. Absent on
    # every well-formed node → no snapshot/diff noise (CHANGED_FIELDS is fixed;
    # content_hash folds only selected fields).
    if raw_depends_on is not None and not isinstance(raw_depends_on, list):
        node["depends_on_invalid"] = raw_depends_on
    return node
...
def _node_from_goal(goal: Dict[str, Any], parent_file: str,
    ...
    return {
        ...
        # Raw pass-through (no `or []`): a falsy scalar (`metrics: 0`) coerced to
        # [] here would hide it from the LIST_FIELDS invalid_type check and
        # mislabel it as goal_without_metric ("no metric at all"). Mirrors
        # _node_from_artifact's metrics/personas raw pass-through; check_goals
        # defers the shape error to invalid_type.
        ...
        # CHANGED_FIELDS already tracks `moscow` directly, and content_hash stays the
        ...
        # Goal keys outside the spec's allowed set (sorted, str-coerced, deduped).
        # The single signal the schema check reads to (a) name a legacy `metric:`
        # for the migrate hint and (b) warn on any other stray goal key — without
        # re-walking the raw frontmatter. `set()` collapses the case where two
        # distinct dict keys str-coerce to the same text (a bare `1:` beside a
        # quoted `"1":`), so check_goals warns once per stray key, not per key
        # occurrence.
...
def index_artifacts(artifacts: List[Dict[str, Any]]) -> Dict[str, Dict[str, Any]]:
    ...
    A SENTINEL id (`<missing-id>`/`<invalid-id>`) is disambiguated by appending the
    artifact's file path: two id-less artifacts would otherwise coerce to the exact
    same sentinel key and the second would silently overwrite the first (dropping
    it from the index though `graph["nodes"]` still lists both). A real id never
    collides — this only widens the key for the already-broken sentinel case."""
    out: Dict[str, Dict[str, Any]] = {}
    for a in artifacts:
        if not a.get("ok"):
            continue
        aid = _scalar_id((a.get("frontmatter") or {}).get("id"))
        if aid in ID_SENTINELS:
            aid = f"{aid}::{a.get('file')}"
        out[aid] = a
    return out
    ...
    for a in artifacts:
        if a["ok"] and _node_type(a) == "product":
            ...
            return {
                ...
                # Raw pass-through (no `or []`), matching the PRODUCT node's own
                # `personas` field (_node_from_artifact). A falsy scalar
                # (`personas: 0`) coerced to [] here used to hide the malformed
                # value from this view while check_consistency still flagged the
                # node as invalid_type — two views of the same field disagreeing.
                # render_ascii.persona() already guards isinstance(..., list)
                # before iterating, so a non-list value here degrades to its
                # existing clean "(no personas yet)" state instead of crashing.
                "personas": fm.get("personas"),
            }
    return {}
...
def _risks(artifacts: List[Dict[str, Any]], product_dir: Path) -> List[Dict[str, Any]]:
    ...
    for a in artifacts:
        if not a["ok"]:
            continue
        fm = a["frontmatter"]
        _risks_v = fm.get("risks")
        for r in _risks_v if isinstance(_risks_v, list) else []:
...
def _competitors(artifacts: List[Dict[str, Any]]) -> List[Any]:
    ...
    for a in artifacts:
        ...
        if _node_type(a) != "brd":
            continue
        _comp_v = fm.get("competitors")
        for c in _comp_v if isinstance(_comp_v, list) else []:
...
def matching_child_counts(graph: Dict[str, Any]) -> Dict[str, int]:
    ...
    Sentinel ids (`<missing-id>`/`<invalid-id>`) are excluded from `nodes_by_id`: two
    id-less nodes of DIFFERENT types would otherwise collapse onto the same key (dict
    comprehension keeps the last one), so an edge from/to a sentinel id could resolve
    to an unrelated node's type and get counted as a match it is not — a real miscount,
    not just a dropped one. Excluding sentinels means such an edge's type never
    resolves, so it is never spuriously counted either way.

    A REAL (non-sentinel) duplicate id is deliberately NOT defended here: a repeated
    real id is a `dup_id` error the consistency gate already surfaces (see
    `index_artifacts`'s "a real id never collides" note), and every `nodes_by_id`
    lookup across the spec skill relies on that gate rather than re-guarding each site
    — hardening this one alone would only diverge from its 15 siblings while defending
    an input the graph is already blocked on."""
    nodes_by_id = {n["id"]: n for n in graph["nodes"] if n.get("id") not in ID_SENTINELS}
...
# both the `--viz delta` surface and the (unshipped) `--validate` impact-pass
# would move together.
...
# story's AC edit registers as a change (the signal an approved-changed-without-
# a-new-decision-record audit pass would consume); the rest are frontmatter facts.
...
def changed_nodes(current: Dict[str, Any], previous: Dict[str, Any]) -> List[str]:
    ...
    """
    cur = {n["id"]: n for n in current.get("nodes", []) if isinstance(n, dict) and "id" in n}
    prev = {n["id"]: n for n in previous.get("nodes", []) if isinstance(n, dict) and "id" in n}
    out: List[str] = []
...
def diff_graphs(current: Dict[str, Any], baseline: Dict[str, Any]) -> Dict[str, Any]:
    ...
    cur_ids = {n["id"] for n in current.get("nodes", []) if isinstance(n, dict) and "id" in n}
    base_ids = {n["id"] for n in baseline.get("nodes", []) if isinstance(n, dict) and "id" in n}
    # `or {}` alone only guards a FALSY product (None/""); a hand-edited
    # snapshot can carry a truthy NON-dict product (a bare string/int), which
    # would still reach .get() below and raise AttributeError. Guard the type
    ...
    # Guard a scalar `personas: alice` (a hand edit missing the list brackets):
    # iterating the bare string would char-split it into phantom personas and
    # spuriously flag "personas changed" — treat a non-list as no personas.
    _cur_p = cur_p.get("personas")
    _base_p = base_p.get("personas")
    cur_set = {str(p) for p in (_cur_p if isinstance(_cur_p, list) else [])}
    base_set = {str(p) for p in (_base_p if isinstance(_base_p, list) else [])}
    if cur_set != base_set:
...
def write_snapshot(graph: Dict[str, Any], root: Path) -> Path:
    ...
    # `dumps_json` serializes a YAML-typed value such as a `target_date`
    # (PyYAML auto-parses it to `datetime.date`) as an ISO string, and a
    # hand-edited `!!set` as a sorted list rather than the hash-seed-ordered
    # `str(set)`, so an identical graph always hashes to the same filename (the
    # idempotent-write invariant below). Its fail-soft fallback also degrades a
    # hostile hand-edit (exotic dict key, anchor cycle, NaN/Inf) to valid JSON
    # instead of crashing the snapshot write — see `dumps_json`'s docstring for
    # the exact classes it covers.
    body = dumps_json({"snapshot_at": generated_at, **graph}, indent=2, ensure_ascii=False)
    # Neutralize any lone surrogate (a non-UTF-8 filename in a node's `file`
    # field, surrogate-escaped by os.fsdecode) BEFORE both the hash and the write:
    # dumps_json guarantees a valid-JSON string but not a UTF-8-encodable one, and
    # this sink hashes + writes directly (not via the emit_json/write_text_atomic
    # chokepoints), so the raw body would else crash the strict .encode("utf-8").
    # Sanitizing once keeps the filename hash consistent with the on-disk bytes.
...
def _as_id_list(v: Any) -> List[str]:
    """Coerce a frontmatter `depends_on` into a sorted, deduped list of ID strings.
    ...
    Duplicates are collapsed: a hand-edited `depends_on: [PRD-X, PRD-X]`
    (copy-paste) must not produce two dep_dangling / dep_cycle findings for one
    real edge — mirrors serves_resolver.resolve_serves, which already dedupes
    `serves`. The wrong-artifact-type placement is surfaced separately by
    check_consistency."""
    if not isinstance(v, list):
        return []
    return sorted(set(x for x in v if isinstance(x, str)))


def stable_dedup_key(entry: Any) -> str:
    """A crash-safe, order-stable dedup key for a frontmatter list entry.

    Stringifies mapping keys (recursively) BEFORE serializing, so a hand-edited
    mixed-type mapping key -- e.g. a bare `1: note` beside `description: x`,
    which is valid YAML a non-technical PO can type -- cannot raise the
    `'<' not supported between 'int' and 'str'` TypeError that a plain
    `json.dumps(entry, sort_keys=True)` would when it tries to sort the raw
    (mixed-type) keys. `default=str` covers any non-JSON-native value. Two
    byte-identical entries always produce the same key.

    NOT injective across key TYPE: because keys are str-coerced, an int key `1`
    and a string key `"1"` collapse to the same canonical key (and if a single
    mapping carries both, one value is dropped last-wins) -- the same accepted
    non-injectivity as `_stringify_keys`. This is fine for the risk/competitor
    dedup use, whose finding text reads only named string-keyed fields
    (description/impact/... , id/name/threat), so a collision can only occur
    between entries that already agree on every read field and would emit
    identical findings anyway. Do NOT reuse this key on data where an int-vs-str
    key distinction is semantically load-bearing."""
    def _canon(o: Any, _seen: frozenset = frozenset(),
               _budget: Optional[List[int]] = None) -> Any:
        # `_seen` marks a TRUE back-edge cycle -> marker; `_budget` (shared
        # mutable counter, decremented per container) caps total node visits so a
        # shared alias fanned out across siblings (a DAG) cannot blow this up to
        # O(fan**depth) and hang the dedup, mirroring _stringify_keys' cap. Real
        # entries stay far under the cap (byte-identical key); only an alias bomb
        # truncates -- deterministically, since walk order is fixed.
        if _budget is None:
            _budget = [_FINGERPRINT_NODE_BUDGET]
        ...
        if isinstance(o, dict):
            if id(o) in _seen:  # YAML anchor cycle -> marker, not RecursionError
                return "[circular reference]"
            _seen = _seen | {id(o)}
            return {str(k): _canon(val, _seen, _budget) for k, val in o.items()}
        if isinstance(o, (list, tuple)):
            if id(o) in _seen:
                return "[circular reference]"
            _seen = _seen | {id(o)}
            return [_canon(x, _seen, _budget) for x in o]
        if isinstance(o, (set, frozenset)):
            # A YAML `!!set` value iterates in PYTHONHASHSEED-randomized order;
            # `str(set)` would make the key non-deterministic (the same content
            # dedups differently run to run). Sort the canonicalized members by
            # `(type-name, str)` -- a total order that is deterministic AND
            # crash-safe on a mixed int/str set, WITHOUT str-coercing the
            # members themselves (so `{1}` and `{"1"}` stay distinct; unlike a
            # dict key, a set member is not JSON-forced to a string).
            return sorted((_canon(x, _seen, _budget) for x in o),
                          key=lambda c: (type(c).__name__, str(c)))
        return o
    return json.dumps(_canon(entry), sort_keys=True, default=str)
...
# that present valid IDs to the PO (e.g. an unresolved-selection help-text
# surface) subtract these so the PO never sees an internal sentinel as a
...
# Parent-scoped ID grammar (ID_PATTERN_BY_TYPE, COMP_ID_PATTERN) lives in
# id_grammar — the single authoritative home. ID_PATTERN_BY_TYPE is
# re-exported from here for check_consistency (imported above); COMP_ID_PATTERN
# has no consumer in this module and is imported directly from id_grammar by
# check_consistency_competition. template_id_alloc imports ID_PATTERN_BY_TYPE
# from id_grammar directly too — either import path resolves the same object,
# so the grammar cannot drift across the skill.
...
# Neutral labels for a sentinel that surfaces as a REFERENCED value (a well-formed
# carrier whose parent-ref field literally holds a sentinel), where — unlike the
# carrier-id case — there is no file path to rewrite it to.
_SENTINEL_REF_LABEL = {"<missing-id>": "(no id)", "<invalid-id>": "(invalid id)"}


def _scrub_sentinel_refs(value: Any) -> Any:
    """Rewrite every ID sentinel occurrence in a caller-composed string / list /
    dict to its neutral label, so a REFERENCED sentinel never reaches a finding
    (the carrier-id case is handled separately, against the file path)."""
    if isinstance(value, str):
        for _sent, _lbl in _SENTINEL_REF_LABEL.items():
            if _sent in value:
                value = value.replace(_sent, _lbl)
        return value
    if isinstance(value, list):
        return [_scrub_sentinel_refs(v) for v in value]
    if isinstance(value, dict):
        return {k: _scrub_sentinel_refs(v) for k, v in value.items()}
    return value
...
def make_finding(check_id: str, severity: str, node: Dict[str, Any], detail: str, **context) -> Dict[str, Any]:
    ...
    Sentinel hygiene: the internal absent/malformed sentinel (`<missing-id>`/`<invalid-id>`)
    can NEVER reach a PO-facing finding from any checker, however the detail was composed.
    Two arms: (1) when the CARRIER node's own id is a sentinel, `artifact_id` is nulled and the
    sentinel inside `detail` is rewritten to the file path; (2) when a sentinel arrives as a
    REFERENCED value (a well-formed carrier whose parent-ref field is a sentinel, in `detail`
    or `context`), it is rewritten to a neutral label — the carrier has a real file so it is
    not the subject, and the raw internal placeholder must not read as if it were a real id a
    PO could look up."""
    nid = node.get("id")
    if nid in ID_SENTINELS:
        ...
        artifact_id = nid
    detail = _scrub_sentinel_refs(detail)
    return {
        "check": check_id,
        "severity": severity,
        ...
        "file": node.get("file"),
        "detail": detail,
        "context": _scrub_sentinel_refs(context) if context else None,
    }
...
def main() -> int:
    ...
    # emit_json keeps the "always exit 0, emit JSON" contract two ways: it
    # serializes through `dumps_json` (YAML-typed values like `status:
    # 2026-05-29` → ISO string, `!!set` → sorted list, and the hostile
    # hand-edit fallback — see `dumps_json`'s docstring for the covered
    # classes), and a closed downstream pipe (`spec_graph … | head`) ends
    # cleanly rather than dumping a BrokenPipeError traceback.
    emit_json(graph)
    return 0
