...
Design social media images via HTML/CSS rendering + screenshot export. Orchestrates `ui-ux`, `brand`, `design-system`, and browser capture tools.
...
3. **Randomly invoke ONE of:** `/hs:ui-ux` OR `/hs:frontend-design` — for layout, hierarchy, visual balance. Pick one at random each run for design variety.
...
Export each HTML to PNG at exact pixel size — Chrome headless / hs:agent-browser / Playwright / Puppeteer. Always add a 3-5s render delay for fonts/images. Full commands + scripts: `references/social-photos-export.md`.

### Step 6: Verify & Fix Designs

Use Chrome MCP / `chrome-devtools-mcp`, `hs:agent-browser`, `hs:chrome-profile`, or Playwright to visually inspect each exported PNG:
