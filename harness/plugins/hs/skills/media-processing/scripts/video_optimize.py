...
class VideoOptimizer:
    ...
    def get_video_info(self, input_path: Path) -> Optional[VideoInfo]:
        ...
        try:
            ...
            den = float(fps_parts[1]) if len(fps_parts) == 2 else 0
            fps = float(fps_parts[0]) / den if den != 0 else 0
    ...
    def calculate_target_resolution(
        ...
        if not max_width and not max_height:
            return width, height
        if not width or not height:  # malformed stream metadata — cannot scale
            return width, height
    ...
    def optimize_video(
        ...
        try:
            ...
            if output_info and self.verbose:
                ...
                reduction = (1 - output_info.size / info.size) * 100 if info.size else 0
    ...
    def compare_videos(self, original: Path, optimized: Path) -> None:
        ...
        br_change = ((opt_info.bitrate / orig_info.bitrate) - 1) * 100 if orig_info.bitrate else 0
        ...
        size_reduction = (1 - opt_info.size / orig_info.size) * 100 if orig_info.size else 0
