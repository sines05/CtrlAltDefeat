    ...
    def apply_migration(self, migration: Migration, dry_run: bool = False) -> bool:
        ...
        try:
            if self.db_type == "mongodb":
                ...
                # NOTE: PostgreSQL DDL (CREATE/ALTER/DROP) auto-commits the current
                # transaction. If up_sql mixes DDL and DML, a DML failure leaves the
                # DDL committed but the migration record un-inserted — the rollback
                # below cannot undo auto-committed DDL. Structure migrations with DDL
                # statements FIRST, followed by DML.
                with self.conn.cursor() as cur:
