 ...
 * Run: node harness/plugins/hs/skills/worktree/tests/worktree.test.cjs
 */
...
// SCRIPT_PATH: tests/ lives one level below scripts/
const SCRIPT_PATH = path.join(__dirname, '..', 'scripts', 'worktree.cjs');
// STANDALONE_DIR: two dirname() levels above tests/ → skills/ dir (inside the harness git repo)
const STANDALONE_DIR = path.dirname(path.dirname(__dirname));

// ============================================================
// F3 FIX: Build a real throwaway monorepo+submodule fixture
// instead of relying on a hardcoded dev-machine path that does not
// exist in this repo's CI environment.
// ============================================================
let MONOREPO_DIR;
let _monoFixtureRoot; // outer temp dir for fixture cleanup

// Run git with an explicit args array — avoids all shell-quoting pitfalls
function gitArr(dir, args) {
...
function buildMonorepoFixture() {
  // Outer temp dir holds sub-origin repos + the monorepo
  const base = fs.mkdtempSync(path.join(os.tmpdir(), 'wt-mono-'));

  // Per-call git -c flags; pass as leading args to gitArr
  const cfg = [
  ...
  // ---- helper: create a regular (non-bare) git repo with one commit ----
  function makeSubOrigin(name) {
    const dir = path.join(base, name);
    fs.mkdirSync(dir);
    gitArr(dir, ['init']);
    fs.writeFileSync(path.join(dir, 'README.md'), `# ${name}\n`);
    gitArr(dir, [...cfg, 'add', 'README.md']);
    gitArr(dir, [...cfg, 'commit', '-m', 'init']);
    return dir;
  }

  const engineerOrigin = makeSubOrigin('claudekit-engineer-origin');
  const cliOrigin = makeSubOrigin('claudekit-cli-origin');

  // ---- create the monorepo ----
  const monoDir = path.join(base, 'claudekit');
  fs.mkdirSync(monoDir);
  gitArr(monoDir, ['init']);
  gitArr(monoDir, [...cfg, 'commit', '--allow-empty', '-m', 'root']);

  // Use relative paths (../name) — protocol.file.allow=always permits the clone
  const engineerRel = path.relative(monoDir, engineerOrigin);
  const cliRel = path.relative(monoDir, cliOrigin);
  gitArr(monoDir, [...cfg, 'submodule', 'add', engineerRel, 'claudekit-engineer']);
  gitArr(monoDir, [...cfg, 'submodule', 'add', cliRel, 'claudekit-cli']);
  gitArr(monoDir, [...cfg, 'add', '.']);
  gitArr(monoDir, [...cfg, 'commit', '-m', 'add submodules']);

  _monoFixtureRoot = base;
  return monoDir;
}

// Build fixture once at startup; tear down on exit
try {
  MONOREPO_DIR = buildMonorepoFixture();
} catch (err) {
  console.error('WARN: monorepo fixture build failed — monorepo tests will skip:', err.message);
  MONOREPO_DIR = null;
}

process.on('exit', () => {
  if (_monoFixtureRoot) {
    try { fs.rmSync(_monoFixtureRoot, { recursive: true, force: true }); } catch {}
  }
});
...
test('info detects monorepo from monorepo root', () => {
  if (!MONOREPO_DIR || !fs.existsSync(MONOREPO_DIR)) return; // Skip if not available
...
test('create dry-run shows explicit base branch source', () => {
  // Use main — the branch guaranteed to exist in this repo
  const result = run('create test-explicit-base --dry-run --json --base main');
  assert(result.success, 'Should succeed with explicit base');
  const json = assertJSON(result.output);
  assert(json.wouldCreate.baseBranch === 'main', 'Should use explicit base branch');
...
test('create in monorepo requires project', () => {
  if (!MONOREPO_DIR || !fs.existsSync(MONOREPO_DIR)) return;
...
test('superproject detection in submodule', () => {
  // Test from claudekit-engineer submodule inside the fixture
  const submodulePath = MONOREPO_DIR ? path.join(MONOREPO_DIR, 'claudekit-engineer') : null;
  if (!submodulePath || !fs.existsSync(submodulePath)) return;
...
// ============================================
// isSafeEnvFileName SECURITY GATE TESTS
// ============================================
console.log('\n🔒 isSafeEnvFileName Security Gate Tests');

// These tests exercise the security gate through the --env flag pathway.
// Unsafe entries must be rejected with a warning; safe entries must pass.

test('isSafeEnvFileName: rejects path traversal (../.env)', () => {
  const result = run('create safe-test --prefix feat --dry-run --json --env "../.env"');
  assert(result.success, 'Dry-run should succeed');
  const json = assertJSON(result.output);
  assert(Array.isArray(json.warnings), 'Should have warnings');
  assert(json.warnings.some(w => w.includes('unsafe env file') && w.includes('../.env')),
    'Should warn about path traversal');
});

test('isSafeEnvFileName: rejects absolute path', () => {
  const result = run('create safe-test2 --prefix feat --dry-run --json --env "/etc/passwd"');
  assert(result.success, 'Dry-run should succeed');
  const json = assertJSON(result.output);
  assert(Array.isArray(json.warnings), 'Should have warnings');
  assert(json.warnings.some(w => w.includes('unsafe env file') && w.includes('/etc/passwd')),
    'Should warn about absolute path');
});

test('isSafeEnvFileName: rejects file with subdirectory', () => {
  const result = run('create safe-test3 --prefix feat --dry-run --json --env "secrets/.env"');
  assert(result.success, 'Dry-run should succeed');
  const json = assertJSON(result.output);
  assert(Array.isArray(json.warnings), 'Should have warnings');
  assert(json.warnings.some(w => w.includes('unsafe env file') && w.includes('secrets/.env')),
    'Should warn about subdir entry');
});

test('isSafeEnvFileName: accepts .env.local (safe name)', () => {
  // .env.local is safe but will not exist in the repo — that's a different warn; no unsafe-entry warn
  const result = run('create safe-test4 --prefix feat --dry-run --json --env ".env.local"');
  assert(result.success, 'Dry-run should succeed');
  const json = assertJSON(result.output);
  // Should NOT warn "unsafe env file" for .env.local
  const hasUnsafeWarn = json.warnings && json.warnings.some(w => w.includes('unsafe env file') && w.includes('.env.local'));
  assert(!hasUnsafeWarn, 'Should not warn unsafe for .env.local');
});
