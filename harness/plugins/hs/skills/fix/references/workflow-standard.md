...
**Pattern:** In SINGLE message, launch 2-3 Explore agents — pin `model:"haiku"` on each (Explore inherits the session's Opus, wasteful for file-finding):
```
Task("Explore", "Find [area1] files related to issue", "Scout area1", model="haiku")
Task("Explore", "Find [area2] patterns/usage", "Scout area2", model="haiku")
Task("Explore", "Find [area3] tests/dependencies", "Scout area3", model="haiku")
```
...
2. Activate `hs:debug` skill. Use `@debugger` subagent if needed.
...
- Apply `hs:problem-solving` skill if stuck
...
4. **Defense-in-depth:** Apply prevention layers where applicable.
...
- If the fix is part of a plan, activate `hs:project-management` for task sync-back and plan status updates
- Update docs if needed via `@docs-manager`
- Ask to commit via `@git-manager` subagent
- Optional: `/hs:journal` to log decisions
...
| 2 | `hs:debug`, `hs:sequential-thinking`, `@debugger` subagent, parallel `Explore`, (`hs:problem-solving` auto) |
| 3 | `hs:problem-solving` (if stuck), `hs:sequential-thinking` (complex logic) |
| 4 | `@tester` subagent, parallel `Bash` verification |
| 5 | `@code-reviewer` subagent |
| 6 | `hs:project-management`, `@git-manager`, `@docs-manager` subagents |
