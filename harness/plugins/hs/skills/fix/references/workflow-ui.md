...
1. `hs:ui-ux` - Design database (ALWAYS FIRST)
2. `hs:ui-ux` - Design principles
...
python3 "${HARNESS_BIN_ROOT:-.}"/harness/plugins/hs/skills/ui-ux/scripts/search.py "<style>" --domain style
python3 "${HARNESS_BIN_ROOT:-.}"/harness/plugins/hs/skills/ui-ux/scripts/search.py "accessibility" --domain ux
```
...
Analyze screenshots/videos with `hs:ai-multimodal` skill.
...
Use `hs:ui-ux-designer` agent.

`TaskUpdate(T2, status="completed")`

### Step 3: Verify Visually
`TaskUpdate(T3, status="in_progress")`
Screenshot + `hs:ai-multimodal` analysis.
...
Use `hs:agent-browser`, `hs:chrome-profile`, Chrome MCP / `chrome-devtools-mcp`, or project-native browser tests.
...
## Tips
- Use `hs:ai-multimodal` for generating visual assets
