...
Use `hs:scout` agent or parallel `Explore` subagents (spawn each with `model:"haiku"`) to find issue locations.
...
### Step 3: Plan Fix
`TaskUpdate(T3, status="in_progress")` — auto-unblocks when T1 + T2 complete. Use `hs:planner` agent.
