...
const directProjectFlag = getFlag("project");
const projectNameFlag = getFlag("project-name");
const envProjectId = process.env.STITCH_PROJECT_ID;
...
if (directProjectFlag) {
  // Priority 1: --project flag — direct ID, no lookup needed
  projectId = directProjectFlag;
  resolvedProjectName = directProjectFlag;
  ...
  // Priority 3: STITCH_PROJECT_ID env — direct ID, no lookup needed
  projectId = envProjectId;
  resolvedProjectName = envProjectId;
  console.error(`[i] Project: ${projectId} (STITCH_PROJECT_ID env)`);
} else {
...
async function main() {
  try {
    ...
    if (remaining < creditsNeeded) {
      ...
      console.error("[i] Use hs:ui-ux as fallback, or wait until midnight UTC.");
      process.exit(2);
    }
    ...
    if (err.code === "RATE_LIMITED") {
      ...
      q.count = q.limit;
      saveQuota(q);
      console.error("[X] Daily quota exceeded (local tracker synced). Try tomorrow or use hs:ui-ux.");
