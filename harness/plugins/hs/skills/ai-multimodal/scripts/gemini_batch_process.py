    ...
    if task == 'generate' and not files:
        ...
        # Use Imagen 4 API for imagen models
            ...
            if result.get('status') == 'billing_required':
                ...
                # Record fallback in result metadata so callers can audit the model change.
                if isinstance(result, dict):
                    result.setdefault('metadata', {})
                    result['metadata']['model_fallback_from'] = model
                    result['metadata']['model_fallback_reason'] = 'billing_required'
