...
### Contract changes
- [ ] Contract-delta: if the diff changes a function's contract (output shape / null↔throw / required param / side-effect / error type) — did you grep the call-sites and read each direct caller depth-1? (`references/contract-delta.md`)
...
The list below are universal never-flag patterns. **Repo-specific** dismissals do NOT live in this static list — they live in the per-repo dismissals store (`harness/scripts/dismissals_store.py`, `docs/review/dismissals.jsonl`). That store
**records and SHOWS** prior dismissals; it does **NOT auto-suppress**. When a finding matches a stored dismissal the reviewer sees "dismissed before — reason: …" and decides again — a genuinely reintroduced bug still surfaces. Never treat a stored dismissal as a silent mute.
