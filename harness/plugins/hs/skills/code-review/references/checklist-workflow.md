...
Findings use the same severity vocabulary as `references/severity-taxonomy.md` — **Critical / Important / Suggestion**, not a separate Critical/Issues split:

```
Pre-Landing Review: N findings (X critical, Y important, Z suggestion)
...
**Important / Suggestion** (non-blocking):
- [src/api/users.ts:88] Magic number 30 for pagination limit
  Fix: Extract to constant `DEFAULT_PAGE_SIZE = 30`
```

Then write the verdict via `write_review_decision.py` (`references/verdict-and-artifact.md`) — this workflow does not have its own artifact; it feeds the same `review-decision.yaml` the gate reads.
...
If user chose A (fix): apply fixes, commit, then re-run tests before continuing. After resolution, write the artifact (`references/verdict-and-artifact.md`): any unresolved Critical → `BLOCKED`; otherwise `PASS` or `PASS_WITH_RISK`.

## Integration with /hs:ship

The ship pipeline calls this workflow at Step 4. Critical findings block the pipeline via the `review-decision.yaml` verdict (`BLOCKED`); Important/Suggestion findings are included in the PR body but do not block.
