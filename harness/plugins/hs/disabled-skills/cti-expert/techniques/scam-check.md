...
**Method:** WebSearch for `site:virustotal.com "[DOMAIN]"` or fetch via `agent-browser`; use `hs:chrome-profile` only for real logged-in Chrome state:
```
...
4. agent-browser - for JavaScript-heavy pages; hs:chrome-profile only when real user Chrome cookies are required (VirusTotal, Scamadviser)
