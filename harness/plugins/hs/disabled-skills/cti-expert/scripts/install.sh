...
if [[ "$OS" == "windows" ]]; then
  VENV_BIN="${CLAUDE_PLUGIN_ROOT}/skills/.venv/Scripts"
else
  VENV_BIN="${CLAUDE_PLUGIN_ROOT}/skills/.venv/bin"
fi
...
echo "Venv:     ${CLAUDE_PLUGIN_ROOT}/skills/.venv"
...
if [[ ! -f "$VENV_PIP" ]]; then
  echo -e "  ${RED}✘${NC} Venv not found at ${CLAUDE_PLUGIN_ROOT}/skills/.venv"
  echo "  Run: python3 -m venv ${CLAUDE_PLUGIN_ROOT}/skills/.venv"
  exit 1
fi
...
SHARETRACE_DIR="${CLAUDE_PLUGIN_ROOT}/skills/cti-expert/vendor/sharetrace"
