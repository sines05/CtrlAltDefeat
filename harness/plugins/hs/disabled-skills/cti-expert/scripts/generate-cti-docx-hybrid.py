...
def ensure_deps():
    ...
    if not missing:
        return
    # Only install into a virtualenv — never mutate the system Python (PEP 668).
    if sys.prefix == sys.base_prefix:
        sys.exit(
            "Missing dependencies: %s\n"
            "Run this script inside a virtualenv so deps stay isolated:\n"
            "  python3 -m venv .venv && . .venv/bin/activate\n"
            "  pip install %s\n"
            "then re-run this script." % (", ".join(missing), " ".join(missing))
        )
...
def ensure_pandoc():
    if shutil.which("pandoc") is None:
        sys.exit(
            "pandoc not found. Install it manually:\n"
