...
class PolarWebhookVerifier {
  ...
  verifySignature(payload, headers) {
    ...
    // Check if any signature matches (constant-time where available)
    const isValid = signatures.some(sig => {
      if (sig.version !== 'v1') return false;
      try {
        return crypto.timingSafeEqual(
          Buffer.from(sig.signature), Buffer.from(expectedSignature)
        );
      } catch {
        // Fallback for when lengths differ (timingSafeEqual rejects unequal buffers)
        return false;
      }
    });
