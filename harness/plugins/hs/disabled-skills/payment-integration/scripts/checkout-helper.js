...
class CheckoutHelper {
  ...
  /**
   * HTML-escape a value for safe use inside double-quoted attribute values.
   */
  ...
  static generateHTMLForm(fields, actionUrl) {
    const inputs = Object.keys(fields)
      .map(key => `    <input type="hidden" name="${this.escapeHTML(key)}" value="${this.escapeHTML(fields[key])}" />`)
      .join('\n');
...
<body>
  <form id="payment-form" action="${this.escapeHTML(actionUrl)}" method="POST">
${inputs}
