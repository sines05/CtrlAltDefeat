...
Verify against your installed `google-adk` version before relying on this: recent releases preserve the agent session when an MCP tool returns an error. If a project drops the session after a tool failure, upgrade `google-adk` before adding workaround state recovery.
