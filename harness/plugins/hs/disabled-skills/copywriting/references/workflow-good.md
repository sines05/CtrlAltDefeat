...
1. **Screenshots provided** → Use `hs:ai-multimodal` skill for detailed context analysis
2. **Videos provided** → Use `hs:ai-multimodal` video-analysis for comprehensive context
3. **Research** → Spawn multiple `hs:researcher` agents in parallel:
   - Competitor messaging analysis
   - Target audience insights
   - Industry best practices
   - Product/service details
4. **Scout codebase** → `/hs:scout ext` (preferred) or `/hs:scout` to find relevant files
5. **Plan** → Use `hs:planner` agent to outline copy structure and strategy
6. **Write** → Use `hs:developer` agent to execute plan
