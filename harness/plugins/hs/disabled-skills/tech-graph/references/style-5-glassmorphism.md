...
Frosted glass cards on dark gradient. For product sites, keynotes, and hero sections.
...
SVG cannot do real backdrop-filter — simulate with layered rects:
...
Note: Do NOT use `@import url()` in `<style>` — rsvg-convert cannot fetch external URLs.
...
## Gradient Text (hero labels only)
...
<text fill="url(#text-grad-blue)" font-weight="700" font-size="20">AI Pipeline</text>
```
