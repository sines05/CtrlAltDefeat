...
test('detectPlan resolves phase files under the phases/ subdir layout', () => {
  const nestedDir = '/tmp/test-nested-plan';
  fs.mkdirSync(path.join(nestedDir, 'phases'), { recursive: true });
  fs.writeFileSync(path.join(nestedDir, 'plan.md'), '# Plan\n');
  ['phase-2-core.md', 'phase-1-setup.md'].forEach(f =>
    fs.writeFileSync(path.join(nestedDir, 'phases', f), `# ${f}\n`));

  const result = detectPlan(path.join(nestedDir, 'plan.md'));
  assertTrue(result.isPlan, 'Should be plan');
  assertEqual(result.phases.length, 2, 'Should find 2 phases under phases/');
  assertTrue(result.phases[0].endsWith(path.join('phases', 'phase-1-setup.md')), '1 first');
  assertTrue(result.phases[1].endsWith(path.join('phases', 'phase-2-core.md')), '2 second');
