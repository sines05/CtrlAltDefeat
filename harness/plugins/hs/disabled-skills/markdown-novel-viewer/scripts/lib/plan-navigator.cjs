...
function detectPlan(filePath) {
  ...
  // Phase files under the flat layout (phase-*.md at the plan-dir root) and the
  // current phases/ subdir layout. Collect from both so a phases/-layout plan
  // still resolves its phase files here (not only via the plan.md link path).
  const collected = [];
  const collect = (baseDir) => {
    ...
    for (const f of names) {
      if (f.startsWith('phase-') && f.endsWith('.md')) {
        collected.push({ name: f, full: path.join(baseDir, f) });
      }
    }
  };
  collect(dir);
  const phasesDir = path.join(dir, 'phases');
  if (fs.existsSync(phasesDir)) collect(phasesDir);

  const phases = collected
    .sort((a, b) => {
      const matchA = a.name.match(/phase-(\d+)([a-z]?)/);
      const matchB = b.name.match(/phase-(\d+)([a-z]?)/);
  ...
  return { isPlan: true, planDir: dir, planFile, phases };
}
