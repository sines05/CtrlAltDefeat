...
Extends the SKILL.md Task Contract with optional `Dependencies` and `Evidence` sections for bootstrap use. `Handoff Summary` replaces the Task Contract's `Handoff Log` name — see Handoff below for why.
