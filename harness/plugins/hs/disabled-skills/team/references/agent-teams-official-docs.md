...
The `Agent` tool (formerly `Task`, renamed v2.1.63) spawns teammates. The `name` parameter is what promotes a spawn from a one-shot subagent to a persistent teammate — it is only present in the tool schema when the Agent Teams gate is on. Omit `name` → plain subagent.
...
These are Claude Code platform events. the harness does not ship custom handlers for them by default; team workflows should monitor TaskList and teammate messages unless a project deliberately adds and validates its own handlers.
