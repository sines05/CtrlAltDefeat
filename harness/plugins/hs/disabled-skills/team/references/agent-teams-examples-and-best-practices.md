...
Agent Teams use significantly more tokens than subagents — each teammate is a separate full session with its own context window. The estimates above assume Opus teammates; picking a cheaper per-teammate model (e.g. Sonnet for read-only research/review — model is per-teammate, not locked) lowers them. Use only when parallel exploration + discussion adds clear value. For routine tasks, a single
...
- One team per session (implicit): a second team is rejected with "one team per session"; there is no `TeamCreate`/`TeamDelete` (removed CC v2.1.178)
